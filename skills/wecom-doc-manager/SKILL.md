...
| `851003` | `no authority` | 无权限调用该接口，**智能表格写入场景**下通常是企业可见范围 > 10 人的规模限制 | 若发生在需要写入数据的场景，引导用户走 Webhook 兜底方案，详见 [webhook-fallback.md](../wecom-smartsheet-data/references/webhook-fallback.md)；其他接口则按权限问题排查 |
